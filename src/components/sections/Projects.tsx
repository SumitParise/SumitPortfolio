import useScrollReveal from '../../hooks/useScrollReveal';
import { PORTFOLIO } from '../../data/portfolio';

const Projects = () => {
  const containerRef = useScrollReveal<HTMLDivElement>({ stagger: 0.1 });

  // Format index (e.g., 0 -> "01", 1 -> "02")
  const formatIndex = (index: number) => `0${index + 1}`.slice(-2);

  return (
    <section
      id="projects"
      ref={containerRef}
      className="py-20 md:py-32 px-6 md:px-12 max-w-6xl mx-auto overflow-hidden"
    >
      <div className="flex flex-col items-center text-center mb-16">
        <div className="flex items-center gap-2 mb-4 justify-center">
          <span className="w-10 h-[2px] bg-[#6C63FF]"></span>
          <span className="font-mono text-sm tracking-widest text-[#6C63FF] uppercase font-semibold">
            Projects
          </span>
          <span className="w-10 h-[2px] bg-[#6C63FF]"></span>
        </div>
        
        <h3 className="text-3xl md:text-5xl font-heading font-extrabold text-white mb-4">
          My Work
        </h3>
      </div>

      {/* Grid structure */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {PORTFOLIO.projects.map((project, idx) => (
          <article
            key={idx}
            className="reveal-item project-card p-6 md:p-8 rounded-2xl flex flex-col justify-between h-full group relative"
          >
            <div>
              {/* Header: Project Index and Links */}
              <div className="flex items-center justify-between mb-6">
                <span className="font-heading font-black text-3xl md:text-4xl text-[#E8E8F0]/15 group-hover:text-[#00D4FF]/25 transition-colors duration-300">
                  {formatIndex(idx)}
                </span>
                
                {/* External links */}
                <div className="flex items-center gap-4 text-[#6B6B80]">
                  {project.link && (
                    <a
                      href={project.link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:text-[#00D4FF] transition-colors duration-300"
                      aria-label="GitHub Repository"
                    >
                      <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                      </svg>
                    </a>
                  )}
                  {project.live && (
                    <a
                      href={project.live}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:text-[#00D4FF] transition-colors duration-300"
                      aria-label="Live Demo Link"
                    >
                      <svg className="w-5 h-5 fill-none stroke-current" viewBox="0 0 24 24">
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          strokeWidth="2"
                          d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"
                        />
                      </svg>
                    </a>
                  )}
                </div>
              </div>

              {/* Title */}
              <h4 className="text-xl font-heading font-bold text-white mb-3 group-hover:text-[#00D4FF] transition-colors duration-300">
                {project.title}
              </h4>
              
              {/* Description */}
              <p className="text-[#6B6B80] font-sans text-sm leading-relaxed mb-6">
                {project.description}
              </p>
            </div>

            {/* Tech Tags */}
            <div className="flex flex-wrap gap-2 mt-4">
              {project.tech.map((techItem) => (
                <span
                  key={techItem}
                  className="px-2.5 py-1 rounded bg-[#0a0a0f] text-[#6B6B80] border border-[#1E1E2E] font-mono text-[10px] tracking-wide"
                >
                  {techItem}
                </span>
              ))}
            </div>
          </article>
        ))}
      </div>
    </section>
  );
};

export default Projects;
